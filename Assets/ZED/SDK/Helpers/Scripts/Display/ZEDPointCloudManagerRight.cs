...
/// </summary>
public class ZEDPointCloudManagerRight : MonoBehaviour
{
    /// <summary>
    ...
    void Update()
    {
		if (zed.IsCameraReady) //Don't do anything unless the ZED has been initialized. 
        {
            if (numberPoints == 0)
            {
                ...
                XYZTexture = zed.CreateTextureMeasureType(sl.MEASURE.XYZ_RIGHT);
                colorTexture = zed.CreateTextureImageType(sl.VIEW.RIGHT);
    ...
    void OnRenderObject()
    {
        if (mat != null)
        {
            foreach (Camera cam in hiddenObjectFromCamera)  //Don't draw anything in other cameras (this part was not included in original code)
